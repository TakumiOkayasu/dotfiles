...
- 「どう設計するか」「どっちが良い」「何を選ぶ」「方針確認」「設計レビュー」 → consultation skill を読み、最低 2-3 案を比較表で提示してユーザーの判断を待つ。実装に着手しない
- 単一案で進めない。実装系の依頼でも複数アプローチがあれば短く比較を出してから着手する
- 「相談モード」相当の語 (どう / 何を / 比較 / 方針 / レビュー / 確認) を検出したら設計フェーズと判定する
...
- 完了報告はテスト pass のログを示してから行う。未確認の「できました」は禁止。テスト未実行・未検証の段階では「完了」と断定せず、状態 (未実行/未確認) を明示した中間報告に留める (→ 実行と検証節)
