...
- 「どう実装するか」「実装して」「修正して」 → fix / feat command を読み、対応
...
- 応答は**日本語**で簡潔に。前置き・締めの定型文 (了解 / ありがとう / 以上です 等) は省く
- 形式はテーブル・箇条書きを優先し、状態は記号で示す (✅ 完了 / ❌ 失敗 / ⚠️ 注意)
- 1 応答あたり 5M token 以内を目安とする
- 不要な出力は**絶対禁止**
...
- 不必要な前置きはしない。反論すべき点は率直に指摘する
- 同意は根拠とセットでのみ示す。追従句で応答を始めない
- コンテキストが積まれるほどこの規律は薄れやすい。長いセッションほど意識的に守る
- **思考は英語で行い、ユーザーとは日本語でやり取りしてください**

### 秘密情報

- パスワード・API キー・トークンをコマンド文字列に平文展開しない
- `.env.example` 等に記載してある変数経由で参照する
  - コマンド難読化 (base64/eval/curl|sh・変数連結コマンド名等) も hook で未検出手法を含め禁止
...
- 言語バージョンは最新の `LTS` / `latest` を使う
- **CLAUDE.md 安定化**: `$HOME/.claude/CLAUDE.md` はプロンプトキャッシュのプレフィックスとして機能する。頻繁な編集はキャッシュミスを引き起こし TPM を浪費する
  - 動的・プロジェクト固有の内容**はプロジェクト側の** `CLAUDE.md` に書き、`$HOME/.claude/CLAUDE.md` は 1 週間単位で安定運用をする
- コメントは、端的かつ本質的な内容、WHY を記入する
