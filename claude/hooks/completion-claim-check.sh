...
MSG="⚠️ [完了報告チェック] 完了報告「${CLAIM}」を検出しましたが、直近の応答にテスト/検証コマンドの実行痕跡がありません。global CLAUDE.md の方針: テスト pass のログを示してから完了報告する。テストを実行して pass を確認するか、検証をスキップした理由を明示してください。"
