...
- 差分が、近接する rule、command-safety、プロジェクト規約で「禁止」「絶対に実行しない」「明示承認必須」と定義された操作を追加・変更・到達可能化する場合、AIレビューだけで `PASS`、approve、merge可と判定しない
- 同等の扱いとする対象は、破壊的または不可逆な操作、権限・認証認可の変更、秘密情報の処理、本番データ・本番設定への影響、依存追加・更新、DBスキーマ変更、deploy/publish、外部書き込み、guardや承認経路の削除を含む
- 該当時はレビュー結果の先頭へ `HUMAN_REVIEW_REQUIRED` を出し、該当箇所、意図、発火条件、到達可能性、影響範囲、rollback、safeguard、代替案、既存の人間承認記録を示す
- テスト、lint、静的解析、AIレビューが成功しても人間ゲートは解除されない。承認記録が確認できない限りマージ判定は `BLOCK`
- 単に危険な文字列が存在するだけで判定せず、差分による新規追加、意味変更、guard削除、実行範囲拡大、到達可能性の変化を実コードで確認する
