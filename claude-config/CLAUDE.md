...
タスク完了後、ブランチがマージされた場合:

1. **メインブランチの確認**
   - `git config --local --get claude.mainBranch` で保存済み設定を確認
   - 設定がない場合:
     - `git branch -a` でブランチ一覧を表示
     - ユーザーに「このプロジェクトのメインブランチはどれですか? (一覧から選んでください)」と尋ねる
     - 回答を `git config --local claude.mainBranch {ブランチ名}` で保存

2. **ブランチ削除の確認**
   - ユーザーに「マージされたブランチ [{ブランチ名}] を削除しますか?」と尋ねる
   - 削除する場合のみ、次のステップへ進む

3. **削除手順**
   - メインブランチに移動: `git checkout {メインブランチ名}`
   - ローカルブランチを削除: `git branch -d {ブランチ名}`
   - 削除完了を報告

**注意事項:**
- メインブランチに移動してから削除すること
- `-d` オプションでマージ済みブランチのみ削除(安全のため)
- ユーザーの許可なしに削除しない
