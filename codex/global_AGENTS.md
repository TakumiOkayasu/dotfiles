...
1. `.codex/progress.md` を読む。未完了タスクがあればそこから再開する
...
リンター・型チェッカー・テスト・スクリプトはいずれも上記の方式で実行する。実行方法はプロジェクトの `codex.md` / `package.json` scripts / `pyproject.toml` / Dockerfile から確認し、指定がなければユーザーに確認する。すべてパスしたことを確認してから完了報告する (未確認での完了報告はしない)。
...
`~/.codex/` 配下のリソースを活用し、既存で実現できる処理を自前実装しない。
...
Codex は `AGENTS.md` 内の `@path` import を展開しないため、必要なルールは明示的に読むか、AGENTS.md に要点を直接集約する。
| リソース | 場所 | 用途 |
| --- | --- | --- |
| skills | `~/.codex/skills/` | オンデマンドの作業手順。タスクに応じて読む |
| prompts/commands | `~/.codex/prompts/commands/` | Codex で貼り付けて使うプロンプト断片 |
| hooks | `~/.codex/hooks/` | 自動処理 (機械的 enforce) |

skills はタスク着手前に該当するものを確認して読む。slash command 風の起動が必要な場合は、`codex-feat` / `codex-fix` / `codex-deep-review` / `codex-commit` または `codex-cmd <name>` を使う。
...
- commit / push は codex では実行せず、作業完了後にユーザーへ依頼する
- PR マージ確認後は `git-cleanup-branch` でローカルブランチを削除する

## 📋 PROGRESS.md (セッション継続)

`.codex/progress.md` でセッションをまたいだ作業継続と判断履歴を保持する。
