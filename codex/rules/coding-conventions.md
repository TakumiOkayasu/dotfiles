...
| ルール | 内容 |
|--------|------|
| **意味のある検証** | アサーションは**具体値**を検証し、実装の振る舞いを実際に判定するテストのみ書く。トートロジー・`toBeDefined()` のみ・カバレッジ稼ぎは禁止。**書く前に「このテストは何を検証しているか?」を 1 行で言語化できるか自問する** |
...
| 詳細は | `codex/skills/tdd/SKILL.md` 参照 |
