...
echo ""
echo "📋 REMINDERS:"
echo "  1. Read AGENTS.md before starting any task"
...
echo "  5. Check codex/rules/ and codex/skills/"
echo ""

if [ -n "$CONFIG_INFO_SCRIPT" ]; then
    "$CONFIG_INFO_SCRIPT" --all
else
    echo "⚠️  codex-config-info.sh not found"
    echo "   Expected: ~/.codex/bin/codex-config-info.sh"
    echo ""
fi
...
SKILLS_UPDATE="$CODEX_DIR/bin/skills-update.sh"
