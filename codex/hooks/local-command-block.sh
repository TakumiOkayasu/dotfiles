...
#
# デバッグ:
#   CODEX_HOOK_DEBUG=1 で /tmp/codex_hook_debug.log にログ出力

# set -e もtrap ERRも使わない（POSIX sh非互換 + exit 1 = 許可扱いリスク）
# 代わりに各操作で明示的にエラーチェックし、失敗時は安全側（ブロック: exit 2）にフォールバック

# --- デバッグログ ---
DEBUG_LOG="/tmp/codex_hook_debug.log"
debug_log() {
    [ "${CODEX_HOOK_DEBUG:-0}" = "1" ] || return 0
...
# CODEX_HOOK_TEST_MODE=1 はテストハーネス専用（本hookがDocker内テストで常時スキップされるのを防ぐ）
if [ "${CODEX_HOOK_TEST_MODE:-0}" != "1" ]; then
