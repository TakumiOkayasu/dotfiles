...
レビュー対象が**30 テスト以上 かつ 複数モジュール (2 モジュール以上) にまたがる**場合は、優先度別 (P1 / P2 / P3) に独立 subagent を 3 並列で dispatch する。テスト数が閾値未満、または単一モジュールに収まる場合は親 context で逐次スキャンする。dispatch 形式・並列起動の作法は `~/.codex/SUBAGENTS.md` を参照:
