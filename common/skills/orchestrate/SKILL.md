...
| `$ARGUMENTS` | 計画の保存先パス (省略時は `.claude/plans/YYYY-MM-DD-<topic>.md`) |
...
| WARN (Warning ≥ 3) | 副作用を伴わない修正は自走で対処し、`.claude/notes/{task-id}.md` に対処内容を記録。副作用を伴う対処 (依存追加等) のみ承認を仰ぐ |
...
報告には各 subagent の成果 (`.claude/notes/{task-id}.md` に集約) と、BLOCK で止めた task があればその理由を含める。
